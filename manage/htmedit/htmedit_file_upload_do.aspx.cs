...
public class cnfly_htmedit_file_upload_do : System.Web.UI.Page
{
	...
	protected int page_do()
	{
		...
		try
		{
			string strfile = upload_save("file", Server.MapPath("/") + "picture\\" + objinfo["key"] + "\\", "", 0);
			if(strfile == "")
			{
				intreturn = 2;
			}
	...
	string upload_save(string partype, string parpath, string parname, int parposition)
	{
		...
		try
		{
			...
			{
				if(strextendname != ".rar" && strextendname != ".zip" && strextendname != ".doc" && strextendname != ".xls" && strextendname != ".pdf")
				{
					return "";
				}
			}
